...
output_dir = 'D:\\대학원\\논문\\textrank\\rawdata\\dblp_v14.tar\\processed_chunks1002\\'
...
def preprocess_row(row):
    for column in ['abstract', 'title']:
        ...
        processed_sentences = []  # 문장별로 저장할 리스트
        ...
        for sentence in doc.sentences:
            ...
            # 전처리된 단어를 다시 문장으로 결합
            if processed_words:
                processed_sentences.append(" ".join(processed_words))  # 문장별로 저장
                
        # 문장 구분을 위해 각 문장 끝에 마침표 추가하여 저장
        row[column] = ". ".join(processed_sentences) + "." if processed_sentences else ""
