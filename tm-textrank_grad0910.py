...
#### 17. textrank + term frequency, term postion, word co-occurence + Infomap + NetMRF
df_filtered_infomap_NetMRF_tpc = df_filtered.copy()
df_filtered_infomap_NetMRF_tpc.dtypes
...
# DataFrame에 적용 (NetMRF를 사용하여 키워드 추출)
df_filtered_infomap_NetMRF_tpc['num_keywords'] = df_filtered_infomap_NetMRF_tpc['keywords'].apply(count_keywords)

df_filtered_infomap_NetMRF_tpc['extracted_keywords'] = df_filtered_infomap_NetMRF_tpc.apply(
...
# num_keywords 열 제거
df_filtered_infomap_NetMRF_tpc.drop(columns=['num_keywords'], inplace=True)

# 결과 출력 (처음 5행)
print(df_filtered_infomap_NetMRF_tpc[['abstract', 'keywords', 'extracted_keywords']])

# Precision, Recall, F1 계산
df_filtered_infomap_NetMRF_tpc['metrics'] = df_filtered_infomap_NetMRF_tpc.apply(
    lambda row: calculate_metrics(row['extracted_keywords'], row['keywords'].split()), axis=1
)
df_filtered_infomap_NetMRF_tpc[['precision', 'recall', 'f1']] = pd.DataFrame(df_filtered_infomap_NetMRF_tpc['metrics'].tolist(), index=df_filtered_infomap_NetMRF_tpc.index)

# ROUGE 점수 계산
df_filtered_infomap_NetMRF_tpc['rouge'] = df_filtered_infomap_NetMRF_tpc.apply(
    lambda row: calculate_rouge(row['extracted_keywords'], row['keywords'].split()), axis=1
)
df_filtered_infomap_NetMRF_tpc['rouge1'] = df_filtered_infomap_NetMRF_tpc['rouge'].apply(lambda x: x['rouge1'].fmeasure)
df_filtered_infomap_NetMRF_tpc['rougeL'] = df_filtered_infomap_NetMRF_tpc['rouge'].apply(lambda x: x['rougeL'].fmeasure)

# 최종 결과 추출
df_result_infomap_NetMRF_tpc = df_filtered_infomap_NetMRF_tpc[['precision', 'recall', 'f1', 'rouge1', 'rougeL']]

# 결과 출력
print(df_result_infomap_NetMRF_tpc) 


#### 18. textrank + Infomap + jaccard + Word2Vec, FastText, BERT, ELMo(다른 임베딩으로 시도해보기)
...
print(df_result_infomap_h) #### 7.1. textrank + term frequency, term postion, word co-occurence + Double Negation, Mitigation, and Hedges Weighting + Infomap + Hierarchical
...
print(df_result_infomap_jcd) #### 15. textrank + Infomap + jcd
print(df_result_infomap_NetMRF) #### 16. textrank + Infomap + NetMRF
print(df_result_infomap_NetMRF_tpc) #### 17. textrank + term frequency, term postion, word co-occurence + Infomap + NetMRF
...
means_result_infomap_NetMRF_tpc = calculate_means(df_result_infomap_NetMRF_tpc)
...
means_dict = {
    ...
    "infomap_NetMRF": means_result_infomap_NetMRF,
    "infomap_NetMRF_tpc": means_result_infomap_NetMRF_tpc
}
