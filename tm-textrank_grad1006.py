...
def textrank_infomap_keywords_glove(title, abstract, top_n=5, embeddings=None):
    ...
    # 제목과 초록을 결합하여 텍스트로 처리
    text = title + " " + abstract
    similarity_matrix, sentences = compute_textrank_similarity_matrix(text, embeddings)
    
    # 네트워크 그래프 생성 및 Textrank 적용
    nx_graph = nx.from_numpy_array(similarity_matrix)
    scores = nx.pagerank(nx_graph)
    ranked_sentences = sorted(((scores[i], s) for i, s in enumerate(sentences)), reverse=True)
    
    # Infomap을 사용하여 모듈화 (클러스터링)
    infomap = Infomap()

    # Infomap에 문장들 간 유사도를 기반으로 엣지 추가
    for i in range(len(sentences)):
        for j in range(i + 1, len(sentences)):
            if similarity_matrix[i][j] > 0:
                infomap.add_link(i, j, similarity_matrix[i][j])
    ...
    # 각 모듈(커뮤니티)에서 중요한 단어 추출
    module_sentences = {}
    for node in infomap.iterTree():
        if node.isLeaf:
            module_id = node.moduleId
            sentence = sentences[node.physicalId]
            if module_id not in module_sentences:
                module_sentences[module_id] = []
            module_sentences[module_id].append(sentence)
