...
# 파일 경로 설정
file_path = 'D:\\대학원\\논문\\textrank\\rawdata\\dblp_v14.tar\\dblp_v14.csv'
...
# keywords 변수가 "[]"인 행들을 제거
df_filtered = df[df['keywords'] != 'NaN']
df_filtered = df_filtered[df_filtered['title'] != 'NaN']
df_filtered = df_filtered[df_filtered['abstract'] != 'NaN']

df_filtered = df_filtered[df['keywords'] != '[]']
df_filtered = df_filtered[df_filtered['title'] != '[]']
df_filtered = df_filtered[df_filtered['abstract'] != '[]']

df_filtered = df_filtered[df_filtered['keywords'] != ' ']
df_filtered = df_filtered[df_filtered['title'] != ' ']
df_filtered = df_filtered[df_filtered['abstract'] != ' ']


# abstract 변수가 NaN인 행들을 제거
df_filtered = df_filtered.dropna(subset=['keywords'])
...
# 전처리할 열 목록
columns_to_process = ['abstract', 'title', 'keywords']

# 각 열에 대해 전처리 수행
for column in columns_to_process:
    processed_texts = []
    for i, text in enumerate(df_filtered[column]):
        ...
        processed_texts.append(processed_text_str)
        
        # Lemmatized 텍스트 출력
        print("\nLemmatized Text:")
        print(processed_text_str)
    
    # 전처리된 결과를 원래 데이터프레임의 해당 열에 덮어쓰기
    df_filtered[column] = processed_texts

# 전처리된 결과를 일부 출력
print(df_filtered[['keywords', 'abstract', 'title']].head())
